...
export class SearchService {
  ...
  // Parse rating from JSON string format like '{"1":0,"2":0,"3":0,"4":0,"5":2}'
  ...
  async searchItemsByModule(
    ...
    if (this.cacheEnabled) {
      ...
      const cached = await this.cacheService.get(cacheKey);
      if (cached) {
        this.logger.debug(`[searchItemsByModule] Cache HIT: q="${q}", latency=${Date.now() - startTime}ms`);
        return cached;
      }
    }
    ...
    const items = allItems.map(item => {
      ...
      // Get delivery_time from store if not present on item
      let deliveryTime = cleanItem.delivery_time;
      if (!deliveryTime && item.store_id) {
        const storeDetail = storeDetails[String(item.store_id)];
        if (storeDetail?.delivery_time) {
          deliveryTime = storeDetail.delivery_time;
          // Recalculate based on distance if available
          if (distanceKm !== undefined && distanceKm !== null) {
            const travelTimeMinutes = this.calculateTravelTime(distanceKm);
            deliveryTime = this.recalculateDeliveryTime(storeDetail.delivery_time, travelTimeMinutes);
          }
        }
      }
    ...
    if (this.cacheEnabled) {
      const cacheKey = this.cacheService.buildCacheKey({ type: 'items', q, ...filters });
      // Use shorter TTL for category/store-specific searches (more dynamic)
  ...
  async searchStoresByModule(
    ...
    let stores = allStores.map(h => {
      ...
      return {
        ...
        logo: src.logo || null,
        logo_full_url: src.logo ? `https://storage.mangwale.ai/mangwale/store/${src.logo}` : null,
        cover_photo_full_url: coverPhoto ? `https://storage.mangwale.ai/mangwale/store/cover/${coverPhoto}` : null,
        delivery_time: deliveryTime || src.delivery_time || null,
        matchType: h.matchType || 'name', // Preserve matchType from hit object
        // Ensure required fields have safe defaults
        phone: src.phone || null,
        email: src.email || null,
        active: src.active !== undefined ? (src.active === 1 || src.active === true ? 1 : 0) : 1,
        open: src.open !== undefined ? src.open : 1,
        veg: src.veg !== undefined ? src.veg : null,
        non_veg: src.non_veg !== undefined ? src.non_veg : null,
        featured: src.featured !== undefined ? src.featured : 0,
        rating_count: src.rating_count || src.order_count || 0,
        avg_rating: src.avg_rating || this.parseRating(src.rating) || 0,
        rating: this.parseRating(src.rating) || src.avg_rating || 0,
        module_id: src.module_id || null,
        zone_id: src.zone_id || null,
        latitude: src.latitude || src.location?.lat || null,
        longitude: src.longitude || src.location?.lon || null,
      };
    });
        ...
        query: {
          bool: {
            filter: [
              ...
              // Categories store status as boolean, so use true instead of 1
              { term: { status: true } },
            ],
          },
        },
      ...
      parentCategoriesMap.forEach((parentCat, parentId) => {
        ...
        if (!existingCategoryIds.has(parentId)) {
          ...
          if (existingParentIndex >= 0) {
            ...
            // Add children's item counts to parent's direct items (parent may have both direct items and children)
            availableCategories[existingParentIndex].item_count = existingItemCount + parentItemCount;
            this.logger.log(
              `[searchCategoriesByModule] Updated parent category ${parentId} item_count from ${existingItemCount} to ${availableCategories[existingParentIndex].item_count} (added ${parentItemCount} from children)`
            );
          }
        }
      });
