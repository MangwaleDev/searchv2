...
class ReindexService {
  ...
  transformItem(item, storeMap, categoryMap) {
    ...
    const transformed = {
      ...
      veg: Boolean(item.veg && item.veg !== 0 && item.veg !== '0'),
      status: Boolean(item.status && item.status !== 0 && item.status !== '0'),
      ...
      slug: item.slug,
      recommended: Boolean(item.recommended && item.recommended !== 0 && item.recommended !== '0'),
      organic: Boolean(item.organic && item.organic !== 0 && item.organic !== '0'),
      is_approved: Boolean(item.is_approved && item.is_approved !== 0 && item.is_approved !== '0'),
      is_halal: Boolean(item.is_halal && item.is_halal !== 0 && item.is_halal !== '0'),
      is_visible: Boolean(item.is_visible && item.is_visible !== 0 && item.is_visible !== '0'),
  ...
  async reindexStores(moduleType) {
    ...
    const transformed = stores.map(store => {
      ...
      return {
        ...
        zone_id: store.zone_id,
        status: Boolean(store.status && store.status !== 0 && store.status !== '0'),
        active: Boolean(store.active && store.active !== 0 && store.active !== '0'),
        veg: Boolean(store.veg && store.veg !== 0 && store.veg !== '0'),
        non_veg: Boolean(store.non_veg && store.non_veg !== 0 && store.non_veg !== '0'),
        delivery: Boolean(store.delivery && store.delivery !== 0 && store.delivery !== '0'),
        take_away: Boolean(store.take_away && store.take_away !== 0 && store.take_away !== '0'),
        rating: store.rating,
